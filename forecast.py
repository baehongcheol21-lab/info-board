...
THEME_FORMAT = (
    "\n\n[예측 의무 — 세 줄 전부, 형식 그대로]\n"
    "[예측:단기] 방향=상승|하락 확률=0.__ 구간=-_._~+_._     ← 다음 거래일(1일)\n"
    "[예측:중기] 방향=상승|하락 확률=0.__ 구간=-_._~+_._     ← 5거래일 뒤\n"
    "[예측:장기] 방향=상승|하락 확률=0.__ 구간=-_._~+_._     ← 20거래일 뒤\n"
    "· 밑줄(_)은 **네가 판단한 숫자**로 채운다. 자리표시자를 그대로 옮겨 적으면 실격이다.\n"
    "· 세 기간의 방향·확률이 전부 같으면 생각을 안 한 것이다 — 기간마다 근거가 다르면 "
    "값도 달라야 한다.\n"
    "· 확률은 그 방향이 맞을 확률(0.50~0.95). **틀려도 된다 — 겁먹고 0.5만 쓰면 실험 "
    "자체가 무의미해진다.** 근거가 있으면 확신을 실어라. 다만 근거 없이 부풀리면 다음 "
    "회의에서 네 성적표로 돌아온다.\n"
    "· 구간은 그 시점까지의 누적 등락률(%) 예상 범위다.\n"
    "· **네 역할대로 판단하라.** 매수 논거를 맡았으면 그 논거가 맞을 때의 그림을, 매도 "
    "논거를 맡았으면 그 논거가 맞을 때의 그림을 숫자로 적어라. 상대와 똑같은 예측을 내면 "
    "토론을 한 의미가 없다.")
